...
int main(int argc, char **argv) {
    ...
    bool isMaster = draftModel.isMaster();
    ...
    config.maxLen = input.size() + 2048;
    std::vector<std::vector<int>> dinputIDs, inputIDs, tmpIDs;
    std::vector<int> dseqs;
    std::vector<int> seqs;
    std::vector<int> pastLens;
    ...
    for (int i = 0; i < loop; ++i) {
        ...
        dinputIDs = {input};
        auto dseqIDs = draftModel.set_input(dinputIDs, dseqs, config);
        auto infer = draftModel.lookaheadN(lookahead_k, rejectedK, rectInputId);
        for (auto id : infer) {
            for (auto x : id)
                std::cout << x << " ";
            input.insert(input.end(), id.begin(), id.end());
            std::vector<std::string> strs = tokenizer->batchDecode(id, 1);
            std::cout << "'" << strs[0] << "'"<< std::endl;
        }
        //}

        //{
        //Timer lookahead(true, "validate batch");
        //input[input.size() - 3] = 3233;
        //input[input.size() - 2] = 3233;
        input[input.size() - 1] = 3233;
        inputIDs = {input};
        for (auto x : input)
            std::cout << x << " ";
        auto seqIDs = model.set_input(inputIDs, seqs, config);
        auto res = model.validateBatch(lookahead_k, tmpIDs);
        rectInputId = std::get<0>(res);
        rejectedK = std::get<1>(res);
        for (int n=0; n<rectInputId.size(); ++n) {
            std::cout << "rejected " << rejectedK[n] << " rect " << rectInputId[i] << std::endl;
        }
        //}
        
        lookahead_k = 2;

        infer = draftModel.lookaheadN(lookahead_k, rejectedK, rectInputId);
        inputIDs.clear();
        for (auto id : infer) {
        ...
        //inputIDs[0][inputIDs[0].size() - 1] = 3233;
        for (auto x : inputIDs[0])
            std::cout << x << " ";
        seqIDs = model.set_input(inputIDs, seqIDs, config);
        res = model.validateBatch(lookahead_k, inputIDs);
        rectInputId = std::get<0>(res);
        rejectedK = std::get<1>(res);
        for (int n=0; n<rectInputId.size(); ++n) {
            std::cout << "rejected " << rejectedK[n] << " rect " << rectInputId[i] << std::endl;
