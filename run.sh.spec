...
DATA_PATH=/home/llm/
BIN=spec_infer
#DRAFT=${DATA_PATH}/models/Llama-2-7b-chat-cpu
#DRAFT=${DATA_PATH}/models/llama-2-70b-chat-hf-xft
DRAFT=${DATA_PATH}/models/TinyLlama-1.1B-Chat-v1.0-xft

#export FLASH_ATTN_THRESHOLD=1
...
OMP_NUM_THREADS=${nth} mpirun -n 1 numactl -C 0-`expr $nth - 1` -m 0 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE" : \
        -n 1 numactl -C  `expr $nth`-`expr $nth \* 2 - 1` -m 1 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE" : \
        -n 1 numactl -C  `expr $nth \* 2`-`expr $nth \* 3 - 1` -m 2 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE"
...
OMP_NUM_THREADS=${nth} mpirun -n 1 numactl -C 0-`expr $nth - 1` -m 0 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE" : \
        ...
        -n 1 numactl -C  `expr $nth \* 2`-`expr $nth \* 3 - 1` -m 1 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE" : \
        -n 1 numactl -C  `expr $nth \* 3`-`expr $nth \* 4 - 1` -m 1 ./${BIN} -m ${DATA_PATH}/models/${MODEL_NAME} -t ${DATA_PATH}/models/${TOKEN_NAME}/tokenizer.model -l ${SeqLen} --output_len=${OutLen} -d ${DTYPE} -b ${BSIZE} --loop ${LOOP} --num_beams ${NUM_BEAMS} --kv_cache_dtype ${KV_DTYPE} --draft_model ${DRAFT} --no_stream -i "$SAMPLE"
